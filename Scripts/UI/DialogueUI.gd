...
var button_scene = preload("res://Scenes/UI/Buttons/DialogueButton.tscn")
var continue_button_scene = preload("res://Scenes/UI/Buttons/ContinueButton.tscn")
...
func next_passage(id):
	...
	while label.visible_characters < len(label.text):
		...
		if Input.is_action_pressed("ui_accept"): label.visible_characters = len(label.text)
